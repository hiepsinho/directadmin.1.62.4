...
case "$1" in
	...
	if [ "$1" = "auto" ]; then
		...
		${WGET_PATH} -O ${LID_INFO} https://mirrors.trung.pro/services/license/getInfo.txt
		if [ ! -s ${LID_INFO} ]; then
		...
		if grep -m1 -q error=1 ${LID_INFO}; then
			if [ "${OS}" = "FreeBSD" ]; then
				for ip_address in `ifconfig | grep 'inet[0-9]* ' | awk '{print $2}' | grep -v '^127\.0\.0\.1' | grep -v '^::1' | grep -v '^fe80'`; do {
					${WGET_PATH} --bind-address="${ip_address}" -O ${LID_INFO} https://mirrors.trung.pro/services/license/getInfo.txt
...
SERVER=https://mirrors.trung.pro/services/
...
if [ -s ${CB_OPTIONS} ]; then
	if [ `grep -c '^php1_release=' ${CB_OPTIONS}` -gt 1 ]; then
		...
		rm -f ${CB_OPTIONS}
		wget -O /usr/local/directadmin/custombuild/build https://mirrors.trung.pro/services/custombuild/2.0/custombuild/build
	fi
fi
...
if [ $WGET -eq 0 ]; then
	if [ "$OS" = "FreeBSD" ]; then
		...
		if [ "$B64" -eq 1 ]; then
			case "$OS_MAJ_VER" in
				7) pkg_add -r https://$FTP_HOST/services/packages-7.1-release/Latest/wget.tbz
					;;
				8) pkg_add -r https://$FTP_HOST/services/packages-8.0-release/Latest/wget.tbz
					;;
				9) pkg_add -r https://$FTP_HOST/services/packages-9.0-release/Latest/wget.tbz
					;;
			...
			case "$OS_MAJ_VER" in
				7) pkg_add -r https://$FTP_HOST/services/packages-7-stable/Latest/wget.tbz
					;;
				6) pkg_add -r https://$FTP_HOST/services/packages-6-stable/Latest/wget.tbz
					;;
				*) pkg_add -r https://$FTP_HOST/services/packages-7-stable/Latest/wget.tbz
					;;
			esac
		fi
	elif [ "$OS" = "debian" ]; then
...
addPackage()
{
	...
	if [ "$OS" = "FreeBSD" ]; then
		if [ "$B64" -eq 1 ]; then
			case "$OS_MAJ_VER" in
				7) pkg_add -r https://$FTP_HOST/services/packages-7.1-release/Latest/${1}.tbz
					;;
				8) pkg_add -r https://$FTP_HOST/services/packages-8.0-release/Latest/${1}.tbz
					;;
				9) pkg_add -r https://$FTP_HOST/services/packages-9.0-release/Latest/${1}.tbz
					;;
			esac
		else
	                case "$OS_MAJ_VER" in
				7) pkg_add -r https://$FTP_HOST/services/packages-7-stable/Latest/${1}.tbz
					;;
				6) pkg_add -r https://$FTP_HOST/services/packages-6-stable/Latest/${1}.tbz
					;;
	                        *) pkg_add -r https://$FTP_HOST/services/packages-7-stable/Latest/${1}.tbz
	                                ;;
	                esac
		fi
	elif [ "$OS" = "debian" ]; then
...
if [ "$CURLDEV" -eq 0 ]; then
	...
	if [ "${FILES_PATH}" = "es_6.0" ] || [ "${FILES_PATH}" = "es_6.0_64" ]; then
		...
		if [ ! -s /usr/include/curl/curl.h ]; then
			...
			echo "*   https://mirrors.trung.pro/services/es_6.0/libcurl-devel-7.19.7-16.el6.i686.rpm";
			echo "*   https://mirrors.trung.pro/services/es_6.0_64/libcurl-7.19.7-16.el6.x86_64.rpm";
			echo "*";
...
if [ "$OS" = "debian" ]; then
	...
	if [ $RET -eq 1 ]; then
		...
		echo "*******************";
		echo "";
		echo "wget does not support https.  Downloading a new wget for you from https://mirrors.trung.pro/services/services/debian_${OS_VER}/wget";
		echo "";
		echo "*******************";

		$BIN_DIR/wget -O $BIN_DIR/wget2 https://mirrors.trung.pro/services/debian_${OS_VER}/wget
		RET=$?
		if [ $RET -eq 0 ]; then
			...
			echo "  cd /usr/bin";
			echo "  wget -O wget2 https://mirrors.trung.pro/services/services/debian_${OS_VER}/wget";
...
if [ -e $OS_OVERRIDE_FILE ]; then
	...
	EXTRA_VALUE="${EXTRA_VALUE}&os=${OS_OVERRIDE}"
		...
		$BIN_DIR/wget $WGET_OPTION -S --tries=5 --timeout=60 -O $DA_PATH/update.tar.gz $BIND_ADDRESS "https://mirrors.trung.pro/update.tar.gz"
